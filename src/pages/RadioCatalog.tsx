...
const RadioCatalog = () => {
  ...
  const initialSelections = catalogSelectionsFromLocation(routeCountry, routeTag, searchParams);
  const [query, setQuery] = useState(initialSelections.query);
  const [submittedQuery, setSubmittedQuery] = useState(initialSelections.query);
  const [selectedCountries, setSelectedCountries] = useState(initialSelections.countries);
  const [selectedTags, setSelectedTags] = useState(initialSelections.tags);
  const [selectedLanguages, setSelectedLanguages] = useState(initialSelections.languages);
  const [sort, setSort] = useState<StationSort>(initialSelections.sort);
